    ...
    <div className="space-y-3 rounded-xl border border-cs-border bg-cs-surface p-4">
      <div className="flex items-center justify-between">
        ...
        {canRemove && (
          <button
            type="button"
            onClick={onRemove}
            className={cn('text-sm text-cs-danger-text', formFieldFocusRingClassName)}
          >
      ...
      <details>
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          {MACRO_FIELDS.map(({ field, labelKey }, index) => (
            <label key={field} className="block">
              ...
              <input
                type="number"
                inputMode="decimal"
                enterKeyHint={index === MACRO_FIELDS.length - 1 ? 'done' : 'next'}
                autoComplete="off"
                min={0}
                step={0.1}
                value={item[field] || ''}
...
export function ManualMealEntryView({ scanner }: ManualMealEntryViewProps) {
  ...
  return (
    <div className="space-y-4" style={{ paddingBottom: keyboardInset }}>
