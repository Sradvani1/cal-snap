...
- **Unit tests** — eleven range/state assertions in `WeighInTests`
...
flowchart TD
    ...
    NotificationManager -->|"onWeighInReminderTapped(userId) / pending"| WeighInSheet
...
| `CalSnap/Features/Progress/WeighInSheetContext.swift` | Identifiable sheet context (replaces technical-spec `$weighInDraft` naming) |
| `CalSnap/Features/Progress/WeighInView.swift` | Weigh-in sheet UI (`NavigationStack` + `Form`) |
| `CalSnap/Features/Progress/WeighInViewModel.swift` | Draft state, cached preview, `setUseLbs(_:)`, save via `WeighInService` |
| `CalSnap/Features/Progress/WeightProgressView.swift` | Full progress screen with child views + Swift Charts |
| `CalSnap/Features/Progress/WeightProgressViewModel.swift` | Display-only load, cached chart/stats derivations |
| `CalSnapTests/WeighInTests.swift` | Eleven PR6 unit tests |
...
| `CalSnap/Features/Dashboard/DashboardView.swift` | `.sheet(item:)`, progress navigation, notification wiring with `userId`, dual-user reminders |
...
2. **Plateau cadence** — last 3 weigh-ins with ≥6-day spacing (`AppConstants.Plateau.weeklyMinimumDaySpacing`); replaces PR3 count-based proxy
...
5. **Notification tap** — presents weigh-in sheet on dashboard for the notified `userId` (switches active profile when needed); no deep-link navigation push
...
- Cached TDEE/target preview via `refreshPreview()` (invalidated on weight/unit change)
- Save returns `WeighInService.SaveResult`; `onSaved` callback triggers dashboard reload + reminder reschedule
- “Remind me tomorrow” calls `await notificationManager.snoozeUntilTomorrow(userId:)` (permission-checked)
- `.presentationSizing(.form)` on sheet root

### `WeightProgressView`

- Child views: `WeightProgressHeaderView`, `WeightProgressBarView`, `WeightProgressChartView`, `WeightProgressStatsGridView`, `WeightProgressHistoryView`
- Loads via `.task(id: reloadTrigger)`; derived chart/stats cached in view model on `load`
- Empty chart shows “Log your first weigh-in” button
- Stats: lost so far, to goal, weekly rate (or “Need more weigh-ins”), projected goal date (or “Maintaining” / “—”)
- Accessibility: progress bar, chart summary, history row labels

### `DashboardView`

- `WeighInSheetContext` in `Features/Progress/WeighInSheetContext.swift`
- `presentWeighInSheet(for userId:using:)` resolves profile, switches `activeUserId` when needed
- On save: `reloadDashboard()`, `scheduleReminderIfNeeded()`; plateau handled by `loadToday` → `checkForPlateau()`
- Notification: registers `onWeighInReminderTapped { userId in … }`; drains `consumePendingWeighInRequest()` on appear for cold launch
...
- `pendingWeighInSheet` + `pendingWeighInUserId` when callback not yet registered (cold launch)
- Snooze follow-up uses `UNCalendarNotificationTrigger` aligned to next-day reminder time
...
| `testSaveReturnsDidTriggerPlateau()` | `WeighInService.save` returns `didTriggerPlateau == true` for 3 weekly flat weigh-ins |
| `testPlateauTriggeredOnSave()` | 3 weekly identical weigh-ins at −14d/−7d/today → `showPlateauAlert == true` |
| `testWeeklyPlateauFetchSpacing()` | `fetchWeeklyPlateauWeighIns` excludes entries &lt;6 days apart |
| `testSetUseLbsConvertsWeight()` | Unit toggle preserves kg value across lbs/kg switch |
| `testWeeklyLossRateKg()` | Two weigh-ins 7 days apart → ~1 kg/week rate |
| `testProjectionPointsStopsAtGoal()` | Projection series reaches goal weight |
| `testPendingWeighInSheetConsumedOnce()` | Pending consumed once; returns notified `userId` |
| `testNotificationTapPassesUserId()` | Callback receives notification `userId` |
| `testSnoozeBlocksWeeklyReminderTapOnly()` | Weekly tap blocked during snooze; `-snooze` tap opens sheet |

**Regression:** 64 tests total — verified 2026-06-14
...
- [ ] Dual-user: notification tap for User B switches profile and opens User B weigh-in sheet
...
## 12. Remediation pass (2026-06-14)

| Issue | Fix |
|-------|-----|
| Notification tap ignored `userId` | `onWeighInReminderTapped(UUID?)`; `presentWeighInSheet(for:using:)` switches active profile |
| Cold launch lost `userId` | `pendingWeighInUserId` + `consumePendingWeighInRequest()` |
| Snooze notification 24h mismatch | `UNCalendarNotificationTrigger` at next-day reminder time |
| Missing `scheduleReminderIfNeeded` on save | `onWeighInSaved` callback from `DashboardContentView` |
| Redundant `checkForPlateau` in `onSaved` | Removed; `loadToday` already calls `checkForPlateau()` |
| Preview recalc on every body pass | Cached preview in `WeighInViewModel`; derived data cached in `WeightProgressViewModel` |
| `@ViewBuilder` computed properties | Extracted child `View` structs in `WeightProgressView` and `WeightTrendMiniChart` |
| Missing `.presentationSizing(.form)` | Added to `WeighInView` |
| Misplaced constants | `Plateau.weeklyMinimumDaySpacing`, `WeightProjection.maxWeeks` |
| `fetchLatestWeighIns` O(n) | `fetchLimit` with descending sort |
| Accessibility gaps | Chart/bar/TDEE/history/save-in-progress labels |
| HealthKit errors via `print` | `Logger` in `WeighInService` |
| `WeighInSheetContext` in `DashboardView` | Moved to `Features/Progress/WeighInSheetContext.swift` |
...
- [x] Eleven PR6 unit tests; 64 total passing — verified 2026-06-14
- [x] `#Preview` on `WeighInView`, `WeightProgressView`
- [x] Post-review fixes applied (see §11)
- [x] Remediation pass applied (see §12)

---

## 14. Builds on / hands off to
