...
todos:
  ...
  - id: fix-session-cookie
    content: "P1: Migrate to createSessionCookie/verifySessionCookie (WR02-SESS-01)"
    status: pending
  - id: fix-auth-errors
...
| WR02-SESS-02 | P2 | `session-edge.ts` projectId fallback `demo-calsnap` vs `next.config.ts` default `calsnap-web` — only if env unset |

**P1 fix direction for WR02-SESS-01:** Migrate to Firebase Admin `createSessionCookie(idToken, { expiresIn: '5d' })` in POST route; verify with `verifySessionCookie` in [session-edge.ts](calsnap-web/lib/auth/session-edge.ts) and [verify-api-session.ts](calsnap-web/lib/auth/verify-api-session.ts). Keep `onIdTokenChanged` posting fresh session cookies on token refresh.
...
- [ ] Firestore read failure behavior documented (both layouts default to onboarding)
...
1. **Session durability (WR02-SESS-01)** — **locked:** `createSessionCookie(idToken, { expiresIn: '5d' })` in POST; `verifySessionCookie` in edge middleware + API routes; keep `onIdTokenChanged` refresh posting new session cookies
2. **Auth error mapping (WR02-AUTH-01)** — **locked scope:** 5 login/signup codes + existing Google codes → `lib/copy/auth.ts`
3. **Project ID fallback alignment (WR02-SESS-02)** — if confirmed, unify default to env-driven single source
...
| Session route / edge verify tests | Mock admin auth; assert `createSessionCookie` + `verifySessionCookie` paths |
...
7. Residual risks — minAge 16, Google manual-only, session edge cases, P3 deferred
...
| Risk | Mitigation |
|------|------------|
| `createSessionCookie` changes break emulator E2E | Test with `NEXT_PUBLIC_USE_FIREBASE_EMULATOR=true`; extend emulator decode path if needed |
