...
**Baseline result:** Integration failed on Storage emulator config (WR01-CI-02). Fixed with storage target + `.firebaserc` bucket mapping before proceeding.
...
| ------------- | ------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| WR01-CI-02    | **P0** | CI        | Storage emulator: `Must supply 'target' in Storage configuration` — array + `bucket` without `target`                                                         | **Fixed** — `target: "default"` + `.firebaserc` bucket targets                                       |
| WR01-E2E-01   | **P1** | E2E       | Only `helpers/onboarding.ts` + inline mocks in `happy-path.spec.ts`; no shared auth/api-mock/fixture utilities for WR02–WR08                                    | **Fixed** — build helper scaffold (merge-blocking)                                                   |
| WR01-MAP-01   | **P1** | Models    | Zero round-trip unit tests for `mealDocToEntry`/`mealEntryToDoc`, `weighInDocToEntry`/`weighInToDoc`, `profileToDoc`/`docToProfile`                             | **Fixed** — add `tests/unit/model-mappers.test.ts`                                                   |
| WR01-CAL-01   | **P1** | Nutrition | 5 of 13 calculator functions lack direct unit tests: `macroPercents`, `fiberTargetG`, `weeklyLossRateKg`, `projectedGoalDate`, `projectionPoints`               | **Fixed** — expand [nutrition-calculator.test.ts](calsnap-web/tests/unit/nutrition-calculator.test.ts) |
| WR01-INPUT-01 | **P1** | Components | `LocalNumberInput` unmount commit refs desynced — step navigation could drop pending edits | **Fixed** — `useLayoutEffect` ref sync + handler updates |
...
### [docs/implementation/web/PR-WR01.md](docs/implementation/web/PR-WR01.md)

Complete — see repo file for audit checklist, baseline snapshot, findings matrix, E2E helper contract, acceptance criteria, residual risks.
...
- [x] Merge gate green locally and in CI
- [x] Zero open P0/P1 in domain layer (findings matrix all resolved or downgraded with rationale)
- [x] PR-W01 13-case nutrition matrix still passes
- [x] 5 previously untested calculator functions have direct unit tests
- [x] Firestore mapper round-trip tests for meal, weigh-in, profile
- [x] `loginWithEmail` helper implemented and documented (no WR01 login E2E spec — WR02 validates)
- [x] `createOnboardedUser()` returns `{ email, password }` for downstream reuse
- [x] `PR-WR01.md` complete with findings matrix + residual risks
- [x] No real Gemini calls in CI (route mocks + `.env.e2e` dummy key unchanged)
...
| `calsnap-web/.firebaserc`                             | Storage bucket targets              |
...
**Final result (2026-06-30):** All green — 184 unit, 11 integration, 1 e2e.
